...
// SoundEx Function was copied till here

int main()
{
	FILE *fin;
	char *str,fname[501],keyword[501],key_code[501],*word,word_code[501],line[10001],line_cpy[10001];
	scanf("%s %s",fname,keyword);
	SoundEx(key_code,keyword,4,4);
	//printf("%s",key_code);
	...
	while(fgets(line,10000,fin) != NULL)
	{
		...
		word = strtok(line_cpy," ."); //to store words one at a time
		while(word != NULL) //to store words one at a time
		{
			//printf("%s\n",word);
			word = strtok(NULL," .\n\0"); //to store words one at a time
			SoundEx(word_code,word,4,4);
			
			if(strcmp(key_code,word_code) == 0)
			{
				printf("%s",line);
				break;
			}
